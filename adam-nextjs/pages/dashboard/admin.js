...
export default function Admin() {
    ...
    async function handleEmployerDelete(employID, accID){
            let accountData;
            let employerData;
    
            const empbaseUrl = "http://localhost:8080/employer/";
            const empfinUrl = empbaseUrl + employID;
            ...
            await Axios(accfinUrl, {
                ...
                await Axios("http://localhost:8080/account/", {
                    ...
                    const merged = _(employerData).keyBy('accID').merge(_.keyBy(accountData, 'accID')).values().value();
                    console.log(employerData, accountData);
                    console.log(merged);
                    localStorage.setItem("employers", JSON.stringify(merged));
                });
            });
            
            setEmployerData(JSON.parse(localStorage.getItem("employers")));
            console.log(setEmployerData);
    }
                            ...
                            <TableBody>
                                {_.map(employersData, (value, index) =>
                                    ...
                                    <TableRow key={index}
                                        ...
                                        <TableCell align="center"><br /><Stack direction="column" spacing={2}><Button variant="contained" value={value.account_id} onClick={(event)=>handleEmployerUpdate(event.target.value)}>Update</Button><Button variant="contained" value={value.account_id} onClick={(event)=>handleEmployerDelete(value.employID, value.accID)}>Delete</Button></Stack></TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                        </Table>
